...
class MassMergeModels(BaseExtensionProcess):
    def __init__(
        ...
        self.final_path = self.get_conf('final_path', required=True)
        ...
        self.weight_json = self.get_conf('weight_json', required=True)
        self.merge_step = self.get_conf('merge_step', default=1, as_type=int)
    ...
    def run(self):
        ...
        for model_config in tqdm(self.models_to_merge, desc=f"Merging models step {self.merge_step}"):
            if self.merge_step == 2:
            ...
            try:
                ...
                if isinstance(sd_model.text_encoder, list):
                    ...
                    for text_encoder, base_text_encoder in zip(sd_model.text_encoder, base_model.text_encoder):
                        for key, value in text_encoder.state_dict().items():
                            if self.merge_step == 1 or self.merge_step == 3:
                                ...
                                value *= multiplier
                            elif self.merge_step == 2:
                                # just calculate mse from merge model
                                mse_dict[model_config.name_or_path][f"te.{te_num}.{key}"] = mse_loss(value, base_text_encoder.state_dict()[key]).item()
                            else:
                                pass
                        te_num += 1
                else:
                    ...
                    for key, value in sd_model.text_encoder.state_dict().items():
                        if self.merge_step == 1 or self.merge_step == 3:
                            ...
                            mse_dict[model_config.name_or_path][f"te.{key}"] = mse_loss(value, base_model.text_encoder.state_dict()[key]).item()
                        else:
                            pass
                ...
                for key, value in sd_model.unet.state_dict().items():
                    if self.merge_step == 1 or self.merge_step == 3:
                        ...
                        mse_dict[model_config.name_or_path][f"unet.{key}"] = mse_loss(value,  base_model.unet.state_dict()[key]).item()
                    else:
                        pass
        ...
        if self.merge_step != 2:
            ...
            # remove the base model to free memory
            del base_model
